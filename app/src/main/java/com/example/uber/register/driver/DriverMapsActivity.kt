...
import com.example.uber.databinding.ActivityDriverMapsBinding
...
class DriverMapsActivity : AppCompatActivity(), OnMapReadyCallback, LocationListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.exitBtnDriver.setOnClickListener {
            ...
            editor?.apply()
            toast("Good bye!")
            startActivity(Intent(this@DriverMapsActivity, RegisterActivity::class.java))
        }
            ...
            override fun onLocationResult(result: LocationResult) {
                ...
                for (location in result.locations) {
                    ...
                    mMap.addMarker(MarkerOptions().position(driverPosition).title("Your are here!").icon(
                        BitmapDescriptorFactory.fromResource(R.drawable.pin4)))
                    mMap.addMarker(MarkerOptions().position(customerPosition).title("Customer is here!").icon(BitmapDescriptorFactory.fromResource(R.drawable.pin3)))
    ...
    override fun onLocationChanged(location: Location) {
        ...
        val driverPosition = LatLng(location.latitude, location.longitude)
        val customerPosition = LatLng(selectedOrder.location?.latitude!!, selectedOrder.location?.longitude!!)
        mMap.addMarker(MarkerOptions().position(driverPosition).title("Your are here!").icon(BitmapDescriptorFactory.fromResource(R.drawable.pin4)))
        mMap.addMarker(MarkerOptions().position(customerPosition).title("Customer is here!").icon(BitmapDescriptorFactory.fromResource(R.drawable.pin3)))
    }
}